...
- `modes`: Currently unused; these are the modes for Chromatic to use in rendering every story across our various supported contexts.
...
- `testId`: used to render a `data-test-id` value to be used by a testing suite to target specific elements.
- `wrapperStyles` (object): used to pass custom inline CSS to be assigned to the root container of the story.
...
To optimize snapshot usage, we organize our stories into groups when rendered inside the Chromatic environment. This is done by adding an `window.isTestEnv()` check and creating groups of stories (note: do not import the `isChromatic` function directly, there is added functionality we've included when sourcing it from`window`). See example below:

In the `template.js` file:

```js
export const AccordionGroup = ({ customStyles = {}, ...args }, context) => {
  return html`
...
- Documentation purposes, you can use the custom tag:
  ```js
  Default.tags = ["docs-only"];
  ```
- Visual regression testing, you can use the custom tag:
  ```js
  Default.tags = ["vrt-only"];
  ```
