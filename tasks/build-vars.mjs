...
/** --- LEGACY builder for converting tokens to custom properties --- */
...
async function getAllVars(paths) {
	...
	if (files.length === 0) return variables;
	...
	for (const filepath of files) {
		...
		// Determine the file's context
		let context;
		const base = basename(filepath, ".css");
		if (filepath.includes("/globals/")) {
			const category = _.lowerCase(_.startCase(base.split("-").pop()));
			const parts = category.split(" ");
			context = `${parts[1]}/${parts[0]}`;
		} else if (filepath.includes("/scales/")) {
			context = `${base.split("-").pop()}`;
		} else if (filepath.includes("/themes/")) {
			context = `${base.split("-").pop()}`;
		} else if (filepath.includes("/components/")) {
			context = `components/${base.split("-").pop()}`;
		} else {
			context = base;
		}
		...
		postcss.parse(css).walkDecls((decl) => {
			if (variables.has(decl.prop)) {
				...
				variables.set(decl.prop, { [context]: decl.value });
			}
		});
	}
...
async function processAliases(filepath, aliases = []) {
	...
	await fsp
		...
		.then((c) => {
			if (typeof c === "undefined") return;
			content += `\n/* Sourced: ${relative(process.cwd(), filepath)} */\n`;
			content += c.replace(
				/:root/,
	...
	return fsp.writeFile(filepath.replace("/css/", "/dist/"), content);
}
...
async function captureMetadata(paths = []) {
	// Get literally all of the possible vars (even overridden vars that are different between themes/scales)
	const allVars = await getAllVars(paths);
	if (!allVars || allVars.size === 0) return Promise.resolve();
...
async function main() {
	...
	await Promise.all(
		["globals", "themes", "scales", "components", "json"].map(async (dir) => {
			return fsp
	...
	await Promise.all([
		...
		copyGlobals(["spectrum-metadata.json"], {
			cwd: join(process.cwd(), "json/"),
		}),
	])
		.then(
...
// Run this inside of a vars package only
main()
	.then(() => {
