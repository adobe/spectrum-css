...
async function processComponent(component, {
    ...
    const pkg = existsSync(pkgPath) ? require(pkgPath) : { name: `@spectrum-css/${component}` };
    ...
    if (existsSync(join(cwd, component))) {
        ...
        warnings.push(`${`${relative(pathing.root, join(cwd, component))}`.brightYellow} not found locally.\n`);
    }
...
let { _: components, output = join(pathing.root, ".diff-output"), cache = true } = yargs(hideBin(process.argv)).argv;

if (!components || components.length === 0) components = allComponents;
