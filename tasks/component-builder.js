...
async function build({ cwd = process.cwd(), clean = false } = {}) {
	...
	const styles = await fetchContent(["*.css"], {
		cwd,
		shouldCombine: true,
	});
	...
	return Promise.all([
		// This was buildCSS
		processCSS(styles?.[0]?.content, inputPath, path.join(cwd, "dist", "index.css"), { cwd, clean })
			.then(async (reports) =>
				Promise.all([
		...
		processCSS(content, inputPath, path.join(cwd, "dist/index-base.css"), {
			cwd,
			clean,
			lint: false,
		}),
...
async function buildThemes({ cwd = process.cwd(), clean = false } = {}) {
	...
	const contentData = await fetchContent(["themes/*.css", "definitions.css"], { cwd, clean });
	...
	return Promise.all(
		contentData.map(async ({ content, input }) => {
			let outputPath = path.join(cwd, "dist", input);
			if (path.basename(input, ".css") === "definitions") {
				outputPath = path.join(cwd, "dist", "themes", "spectrum.css");
			}
			return processCSS(content, path.join(cwd, input), outputPath, { cwd, clean, lint: false });
		})
	);
}
