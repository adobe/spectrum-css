			...
			fsp.writeFile(
				...
				prettier.format(
					[
						...
						...Object.keys(found).sort().map((result) => `| \`--${result}\` |`),
					].join("\n"),
					{ parser: "markdown" }
				),
				{ encoding: "utf-8" }
	...
	promises.push(
		fsp.writeFile(
			...
			return [
                ...
                `🔍  ${`${Object.keys(found).length}`.underline} modifiable custom propert${found.size === 1 ? "y" : "ies"}`,
                `🔍  ${`${selectors.size}`.underline} selector${Object.keys(found).length === 1 ? "" : "s"}`,
			];
		}).catch((err) => {
			if (!err) return;
