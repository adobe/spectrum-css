...
window.addEventListener('DOMContentLoaded', function() {
  ...
  function loadPage(href) {
    ...
    function handleLoad() {
      ...
      if (req.status === 200) {
        ...
        if (newMainContainer) {
          ...
          handleError(204, 'Content not found', `The page loaded successfully, but doesn't contain documentation content.`);
        }
      }
      else {
        handleError();
    ...
    function handleError(status, text, message) {
...
<div class="spectrum-IllustratedMessage">
  ...
  <h2 class="spectrum-Heading spectrum-Heading--pageTitle spectrum-IllustratedMessage-heading">Error ${status}: ${text}</h2>
  <p class="spectrum-Body--secondary spectrum-IllustratedMessage-description">${message}</p>
</div>
`;
      document.querySelector('title').innerHTML = 'Error: ' + req.status;
    }
