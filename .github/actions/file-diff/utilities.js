...
exports.printChange = function (difference) {
    return difference === 0
        ? `No change 🎉`
        : `${difference > 0 ? "+" : "-"}${exports.bytesToSize(Math.abs(difference))} ${difference > 0 ? "⬆" : "⬇"}`;
};
...
exports.formatData = function (data) {
    return [...data.entries()].reduce((table, [{ byteSize, diffByteSize }, readableFilename]) => {
        ...
        if (diffByteSize !== undefined) {
            ...
            row.push(exports.printChange(diffByteSize - byteSize), exports.printPercentChange(delta));
        }
        return [...table, row];
    }, []);
};
...
exports.getAssetURL = async function ({ artifactName, token }) {
    ...
    const {
        data: { artifacts },
    } = await octokit.request("GET /repos/{owner}/{repo}/actions/artifacts?name={name}", {
