    ...
    packages.forEach(async (packagePath) => {
        ...
        function addToFiles(filePath, { isMain = false, isHead = true, cwd } = {}) {
            ...
            if (!existsSync(fullPath)) {
                core.info(`[${cwd}] ${filePath} does not exist, skipping.`);
                return;
            }
            ...
            core.info(`[${cwd}] ${filePath} added to files with { isMain: ${isMain}, hasChanged: ${hasChanged ? "true" : "false"}, size: ${stat.size ?? 0}}`);
            files.set(filePath, {
                isMain,
                hasChanged,
                size: {
