...
export const Template = ({
  ...
  const nestedPopover = id.split("-").includes("nested");
  ...
  return html`
    ...
    ...customClasses.reduce((a, c) => ({ ...a, [c]: true }), {}),
		...
		>
			...
			${when(withTip, () => position && ["top", "bottom"].some((e) => position.startsWith(e)) ? html`<svg class="${rootClass}-tip" viewBox="0 -0.5 16 9" width="10"><path class="${rootClass}-tip-triangle" d="M-1,-1 8,8 17,-1"></svg>` : html`<svg class="${rootClass}-tip" viewBox="0 -0.5 9 16" width="10"><path class="${rootClass}-tip-triangle" d="M-1,-1 8,8 -1,17"></svg>`)}
		</div>
	`;
};
