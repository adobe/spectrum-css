...
export const FullIconSetTemplate = (args) => {
	return html`
		<div
			style=${styleMap({
				"display": "grid",
				// @todo CSS-959: The following line is currently broken by #storybook-root { inline-size: fit-content; }
		...
		>
			${when(args.setName === "workflow", () => {
				...
				return uiIconsWithDirections.sort().map((iconName) => IconWithLabelTemplate({ ...args, iconName }));
			})}
		</div>
	`;
};
 ...
 */
export const IconWithLabelTemplate = (args) => {
