...
- `.spectrum-colorwheel-path`, `.spectrum-colorwheel-path-borders`, and `.spectrum-colorwheel-colorarea-container` are hard coded in CSS and include token values in custom CSS variables so they can be accessed with JS to and used to calculate these values, `const wheel = document.querySelector(".spectrum-ColorWheel-wheel") getComputedStyle(wheel).getPropertyValue('--_track-width'))`
