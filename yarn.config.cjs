	...
	async constraints({ Yarn }) {
		...
		for (const workspace of Yarn.workspaces()) {
			...
			if (components.includes(workspace.cwd)) {
				...
				workspace.set("scripts.build", "yarn run component:build");
				workspace.set("scripts['build:full']", "yarn build && yarn report");
				...
				workspace.set("scripts.report", "yarn run component:report");
