...
test.serial("should work if there is a semver tag", (t) => {
  ...
  compare(
    ...
    [
      (chunk) => t.regex(chunk, "some more features"),
      (chunk) => t.regex(chunk, "BREAKING"),
    ],
  );
});
...
test.serial("should work with unknown host", (t) => {
  compare(
    ...
    [
      (chunk) => t.regex(chunk, "(http://unknown/compare"),
      (chunk) => t.regex(chunk, "](http://unknown/commits/"),
    ],
  );
});
