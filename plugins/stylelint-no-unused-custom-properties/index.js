...
const rule = (enabled, { allowList = [], ignoreList = [] } = {}, context) => {
  return (root, result) => {
    if (!enabled) return;
      ...
      if (start) {
        while (nextLine) {
          if (nextLine.type === "decl" && nextLine.prop.startsWith("--")) {
            ...
            /^\s*@passthroughs?\s*(\s+start)?$/.test(nextLine.text)
          ) {
            break;
          }
    ...
    root.walkDecls((decl) => {
      ...
      parsed.walk((node) => {
        ...
        if (
          !(ignoreList.length
            ...
            : true) &&
          (allowList.length
            ? allowList.some((regex) => regex.test(varName))
            : true)
        ) {
      ...
      if (decl.prop.startsWith("--") && usedInDecl.size > 0) {
        for (let varName of usedInDecl) {
          const previous = relationships.has(varName)
            ? relationships.get(varName)
            : [];
    ...
    root.walkDecls(/^--/, (decl) => {
      ...
      if (
        ...
        !(ignoreList.length
          ? ignoreList.some((regex) => regex.test(decl.prop))
          : true) &&
        (allowList.length
          ? allowList.some((regex) => regex.test(decl.prop))
          : true) &&
      ...
      if (!usedInProps.has(decl.prop)) {
        ...
        if (context.fix) {
          ...
          // if (!ignoreList.some((regex) => regex.test(decl.prop))) {
          stylelint.utils.report({
            message: messages.expected,
            messageArgs: [decl.prop],
            node: decl,
