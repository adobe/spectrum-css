    ...
    return (root, result) => {
        ...
        root.walkComments((comment) => {
            ...
            if (!start && !end && nextLine) {
                if (nextLine && nextLine.type === "declaration" && nextLine.prop.startsWith("--")) {
            ...
            if (start) {
                while (nextLine) {
                    if (nextLine.type === "decl" && nextLine.prop.startsWith("--")) {
                        allowedPassthroughs.add(nextLine.prop);
                    } else if (nextLine.type === "comment" && /^\s*@passthroughs?\s*(\s+start)?$/.test(nextLine.text)) {
                        break;
                    }
        ...
        root.walkDecls((decl) => {
            ...
            parsed.walk((node) => {
                ...
                if (
                    !(ignoreList.length ? ignoreList.some((regex) => regex.test(varName)) : true) &&
                    (allowList.length ? allowList.some((regex) => regex.test(varName)) : true)
                ) {
