...
export async function bundler() {
	const { code, map, warnings } = await bundleAsync({
		filename: path.join(bundleRoot, "src", "index.css"),
		minify: false,
		resolver: {
			read(filePath) {
			...
			resolve(specifier, from) {
				if (/^@spectrum-css\/tokens$/.test(specifier)) {
					return path.resolve(dirs.root, specifier.replace(/^@spectrum-css\//, ""), "dist/index.css");
				}

				if (/^@spectrum-css\//.test(specifier)) {
					return path.resolve(dirs.components, specifier.replace(/^@spectrum-css\//, ""), "dist/index.css");
				}
	...
	if (warnings && warnings.length > 0) {
		warnings.forEach(warning => promises.push("⚠️ ".yellow + warning));
	}

	if (code) {
		promises.push(
			processCSS(code, path.join(bundleRoot, "src", "index.css"), path.join(bundleRoot, "dist", "index.css"), { cwd: bundleRoot }),
			// Write the minified CSS
			processCSS(code, path.join(bundleRoot, "src", "index.css"), path.join(bundleRoot, "dist", "index.min.css"), { cwd: bundleRoot, minify: true }),
		);
	}

	if (map) {
		promises.push(
			writeAndReport(map, path.join(bundleRoot, "src", "index.css.map"), { cwd: bundleRoot }),
		);
	}
