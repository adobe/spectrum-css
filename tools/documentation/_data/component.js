...
const { getPackageDependencyOrder, getReleaseDate, isComponentPackage, isMigrated } = require('../scripts/utilities.js');
 ...
 * @property {string[]} PageMetadata.dependencyOrder
 ...
 * @property {string} PageMetadata.permalink - The url path to the published component page; should exclude `docs` folder
...
module.exports = async (configData) => {
  ...
  for await (const name of fg.stream('*', {
    ...
    const dependencyOrder = await getPackageDependencyOrder(dependencies, pages).then(order => order
    ...
    for await (const file of fg.stream('metadata/*.yml', { cwd: path, onlyFiles: true, absolute: true })) {
      ...
      /** @type {object} */
      const pageData = await readFile(file, "utf8").then(yaml.safeLoad).catch(console.warn);
