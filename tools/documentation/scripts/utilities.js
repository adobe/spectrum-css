...
async function getPackageDependencyOrder(deps, pages) {
    if (deps.length === 0) return [];
    const depList = {};
    for (const p of deps) {
      const { name, dependencies } = await getDependencies(p, pages);
...
module.exports = { getDependencies, getPackageDependencyOrder, getReleaseDate, isMigrated, isComponentPackage };
