...
const site_buildData = async () => {
	...
	for (const metaPath of await fg(`*/metadata/*.yml`, {
		...
		const component = path
			...
			.split("/")
			.pop();
		const href = `${path.basename(metaPath, "yml")}.html`;
		...
		const pkg = await fsp
			.readFile(path.resolve(path.dirname(metaPath), "../package.json"))
			.then(JSON.parse);
		...
		const compiled = require("pug").renderFile(
			`${__dirname}/templates/siteComponent.pug`,
			{
				...
				id: metadata.id ?? path.basename(metaPath, ".yml"),
				dnaVars: metadata,
				nav: nav.sort((a, b) => (a.name <= b.name ? -1 : 1)),
				pageURL: path.basename(metaPath, ".yml") + ".html",
				releaseDate: date,
				pkg,
			}
		);
